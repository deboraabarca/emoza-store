<?php
/**
 * Main class of Emoza Sites Importer plugin.
 *
 ...
 * Importer Class
 */
class Emoza_Starter_Sites_Importer {
	 ...
	 * Singleton instance
	 *
	 * @var Emoza_Starter_Sites_Import
	 */
	 ...
	 *
	 * @return Emoza_Starter_Sites_Import
	 */
	...
	public function init() {
		...
		$this->demos = apply_filters( 'emwc_register_demos_list', array() );
		...
		add_action( 'wp_ajax_emwc_import_start', array( $this, 'emwc_import_start' ) );
		add_action( 'wp_ajax_emwc_import_clean', array( $this, 'emwc_import_clean' ) );
		add_action( 'wp_ajax_emwc_import_plugin', array( $this, 'ajax_import_plugin' ) );
		add_action( 'wp_ajax_emwc_import_contents', array( $this, 'ajax_import_contents' ) );
		add_action( 'wp_ajax_emwc_import_widgets', array( $this, 'ajax_import_widgets' ) );
		add_action( 'wp_ajax_emwc_import_customizer', array( $this, 'ajax_import_customizer' ) );
		add_action( 'wp_ajax_emwc_import_finish', array( $this, 'ajax_import_finish' ) );

		// Hide wpdb errors for avoid wp_send_json brokens.
		if ( isset( $_POST['action'] ) && $_POST['action'] === 'emwc_import_plugin' && ( WP_DEBUG || WP_DEBUG_DISPLAY ) ) {
			global $wpdb;
			$wpdb->hide_errors();
		}
	...
	public function install_plugin( $plugin_slug ) {
		...
		if ( is_admin() && ! class_exists( 'WP_Upgrader' ) ) {
	 ...
	 * Start import.
	 */
	public function emwc_import_start() {
		...
		if ( ! $demo_id || ! isset( $this->demos[ $demo_id ] ) ) {
			wp_send_json_error( esc_html__( 'Invalid demo id.', 'emoza-starter-sites' ) );
		}

		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to start demo.', 'emoza-starter-sites' ) );
		}

		/**
		 * Action hook.
		 */
		do_action( 'emwc_import_start', $demo_id );
	 ...
	 */
	public function emwc_import_clean() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to clean previous import.', 'emoza-starter-sites' ) );
		}
		 ...
		 * Delete posts.
		 */
		$query  = $wpdb->prepare( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = %s", '_emoza_sites_imported_post' );
		 ...
		 * Delete terms.
		 */
		$query  = $wpdb->prepare( "SELECT term_id FROM {$wpdb->termmeta} WHERE meta_key = %s", '_emoza_sites_imported_term' );
		$result = $wpdb->get_col( $query );
	...
	public function clean_widget_settings() {
		...
		$imported_widgets = get_option( '_emoza_sites_imported_widgets', array() );
	...
	public function clean_customizer_settings() {
		 ...
		 */
		$imported_customizer_mods = get_option( '_emoza_sites_imported_customizer_mods', array() );
		 ...
		 */
		$imported_customizer_options = get_option( '_emoza_sites_imported_customizer_options', array() );
	...
	public function ajax_import_plugin() {
		...
		if ( empty( $slug ) ) {
			wp_send_json_error( esc_html__( 'Unknown slug in a plugin.', 'emoza-starter-sites' ) );
		}

		if ( empty( $path ) ) {
			wp_send_json_error( esc_html__( 'Unknown path in a plugin.', 'emoza-starter-sites' ) );
		}

		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( esc_html__( 'Insufficient permissions to install the plugin.', 'emoza-starter-sites' ) );
		}
		 ...
		 * Action hook.
		 */
		do_action( 'emwc_import_plugin', $slug, $path );
		...
		wp_send_json_error( esc_html__( 'Failed to initialize or activate importer plugin.', 'emoza-starter-sites' ) );
	...
	public function ajax_import_contents() {
		...
		if ( ! isset( $this->demos[ $demo_id ]['import'][ $builder_type ] ) ) {
			wp_send_json_error( esc_html__( 'Invalid builder type.', 'emoza-starter-sites' ) );
		}

		if ( ! isset( $this->demos[ $demo_id ]['import'][ $builder_type ]['content'] ) ) {
			wp_send_json_error( esc_html__( 'The url address of the demo content is not specified.', 'emoza-starter-sites' ) );
		}

		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to import contents.', 'emoza-starter-sites' ) );
		}

		$xml_file_url = $this->demos[ $demo_id ]['import'][ $builder_type ]['content'];

		$xml_file_path = get_transient( 'emwc_importer_data' );
		...
		if ( ! $xml_file_path || ! file_exists( $xml_file_path ) ) {
			...
			if ( is_wp_error( $temp_file ) ) {
				wp_send_json_error( esc_html__( 'Failed to download temporary demo xml file.', 'emoza-starter-sites' ) );
			}
			...
			if ( isset( $download_response['error'] ) ) {
				wp_send_json_error( esc_html__( 'Failed to download demo xml file.', 'emoza-starter-sites' ) );
			}

			// Define the downloaded contents.xml file path.
			$xml_file_path = $download_response['file'];

			set_transient( 'emwc_importer_data', $xml_file_path, HOUR_IN_SECONDS );
		 ...
		 */
		if ( is_admin() && ! class_exists( 'WP_Importer' ) ) {
		 ...
		 * Import Core.
		 */
		require_once EMWC_PATH . 'core/vendor/wp-content-importer-core/WPImporterLogger.php';
		require_once EMWC_PATH . 'core/vendor/wp-content-importer-core/WPImporterLoggerCLI.php';
		require_once EMWC_PATH . 'core/vendor/wp-content-importer-core/WXRImportInfo.php';
		require_once EMWC_PATH . 'core/vendor/wp-content-importer-core/WXRImporter.php';
		require_once EMWC_PATH . 'core/vendor/wp-content-importer-core/Logger.php';
		...
		if ( $content_type === 'placeholder' ) {
  		add_filter( 'emwc_importer.processed.attachment', array( $this, 'convert_attachment_to_placeholder' ) );
		}
		...
		// Set the WordPress Importer core as the importer used in this plugin.
		// More: https://github.com/humanmade/WordPress-Importer.
		$importer = new EMWC_WXRImporter( array(
			'fetch_attachments' => true,
			'default_author'    => get_current_user_id(),
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'emwc_logger_options', array(
			'logger_min_level' => 'warning',
		) );

		// Configure logger instance and set it to the importer.
		$logger            = new EMWC_Logger();
		...
		if ( is_wp_error( $importer ) ) {
			wp_send_json_error( esc_html__( 'An error occurred while importing contents.', 'emoza-starter-sites' ) );
		}
		 ...
		 * Action hook.
		 */
		do_action( 'emwc_import_contents' );
	...
	public function ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'emwc_time_for_one_ajax_call', 300 ) ) {
	...
	public function track_imported_post( $post_id = 0 ) {
		...
		update_post_meta( $post_id, '_emoza_sites_imported_post', true );
	...
	public function track_imported_term( $term_id = 0 ) {
		...
		update_term_meta( $term_id, '_emoza_sites_imported_term', true );
	...
	public function ajax_import_widgets() {
		...
		if ( ! isset( $this->demos[ $demo_id ]['import'][ $builder_type ]['widgets'] ) ) {
			wp_send_json_error( esc_html__( 'No widgets WIE file specified.', 'emoza-starter-sites' ) );
		}

		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to import widgets.', 'emoza-starter-sites' ) );
		}
		...
		if ( 200 != wp_remote_retrieve_response_code( $raw ) ) {
			wp_send_json_error( esc_html__( 'Failed to load widget demo file.', 'emoza-starter-sites' ) );
		}
		...
		$data = map_deep( $data, array( 'Emoza_Starter_Sites_Importer', 'replace_attachment_urls' ) );

		$widgets = new EMWC_Widget_Importer();
		...
		$available_widgets = EMWC_Widget_Importer::available_widgets();
		...
		update_option( '_emoza_sites_imported_widgets', $sidebar_widgets, 'no' );

		do_action( 'emwc_import_widgets', $sidebar_widgets, $widget_instances );
	...
	public function ajax_import_customizer() {
		...
		if ( ! isset( $this->demos[ $demo_id ]['import'][ $builder_type ]['customizer'] ) ) {
			wp_send_json_error( esc_html__( 'The url address of the demo customizer is not specified.', 'emoza-starter-sites' ) );
		}

		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to import customizer.', 'emoza-starter-sites' ) );
		}
		...
		if ( 200 != wp_remote_retrieve_response_code( $raw ) ) {
			wp_send_json_error( esc_html__( 'Failed to load customizer demo file.', 'emoza-starter-sites' ) );
		}
		...
		$customizer = new EMWC_Customizer_Importer();

		// Import.
		$results = $customizer->import( $data );

		if ( is_wp_error( $results ) ) {
			wp_send_json_error( esc_html__( 'An error occurred while importing customizer.', 'emoza-starter-sites' ) );
		}

		/**
		 * Action hook.
		 */
		do_action( 'emwc_import_customizer', $data );
	...
	public function ajax_import_finish() {
		...
		$nav_menu_terms = get_terms( array(
			'taxonomy'   => 'nav_menu',
			'hide_empty' => false,
			'meta_key'   => '_emoza_sites_imported_term',
		) );
		...
		if ( class_exists( 'WooCommerce' ) ) {
			$products = get_posts( array(
				'post_type'      => 'product',
				'posts_per_page' => -1,
				'meta_key'       => '_emoza_sites_imported_post',
			) );
		 ...
		 * Action hook.
		 */
		do_action( 'emwc_finish_import', $demo_id, $custom_import_settings );
...
new Emoza_Starter_Sites_Importer();
