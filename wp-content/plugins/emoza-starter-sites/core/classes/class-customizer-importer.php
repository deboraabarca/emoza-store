 ...
 * Customizer Importer
 */
class EMWC_Customizer_Importer {
	/**
	 * Import customizer.
	 *
	...
	public static function import_customizer_options( $data ) {
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'emoza-starter-sites' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'emoza-starter-sites' )
			);
		}

		// Import images.
		if ( apply_filters( 'emwc_customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Unauthorized action.', 'your-text-domain' ) );
			}

			// Require modified customizer options class.
			if ( is_admin() && ! class_exists( 'WP_Customize_Setting' ) ) {
				require_once ABSPATH . 'wp-includes/class-wp-customize-setting.php';
			}

			if ( ! class_exists( 'EMWC_Customizer_Option' ) ) {
				require_once EMWC_PATH . 'core/classes/class-customizer-option.php';
			}

			foreach ( $data['options'] as $option_key => $option_value ) {

				$option = new EMWC_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
		...
		$use_wp_customize_save_hooks = apply_filters( 'emwc_enable_wp_customize_save_hooks', false );
		...
		if ( ! empty( $data['mods'] ) ) {
			update_option( '_emoza_sites_imported_customizer_mods', $data['mods'] );
		}

		if ( ! empty( $data['options'] ) ) {
			update_option( '_emoza_sites_imported_customizer_options', $data['options'] );
		}
		...
		foreach ( $mods as $key => $val ) {
			if ( self::customizer_is_image_url( $val ) ) {
				...
				if ( ! is_wp_error( $data ) ) {
					...
					update_post_meta( $data->attachment_id, '_emoza_sites_imported_post', true );
